...
const payload = {
    ...
    exports.logout = async(req, res, next) =>{
        try {
            let token = req.headers?.unauthorization?.split(" ")[1];
            if(!token) token = req.cookie?.bflux;               // getting access token from the cookie
            if(!token) token = req.headers?.cookie?.split("=")[1]; // getting acces token fron the header
            const {refreshToken} = req.body; // getting refresh token from the body
            ...
            if(!!foundUser){
                jwt.verify(refreshToken, config.REFRESH_TOKEN_SECRET, async(err, decoded)=>{
                    ...
                    const userToken = await AccountModel.findOne({__id:decoded.id}).exec();
            ...
            jwt.verify(refreshToken, config.REFRESH_TOKEN_SECRET, async(err, decoded)=>{
                ...
                if(err || foundUser.__id.toString() !== decoded.id) return next(APIError.unauthenticated("Token expired"));
            })
